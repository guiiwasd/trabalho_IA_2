...
minhaMiniVGGNetModel = model.fit(XTreino, yTreino, batch_size=batchSize, epochs=epochs, validation_data=(XTeste, yTeste))
...
f, ax = plt.subplots()
ax.plot(minhaMiniVGGNetModel.history-['accuracy'], 'o-')
ax.plot(minhaMiniVGGNetModel.history['val_accuracy'], 'x-')
ax.legend(['Acurácia no Treinamento', 'Acurácia na Validação'], loc=0)
ax.set_title('MiniVGGNet - Acurácia por Época')
ax.set_xlabel('Época')
